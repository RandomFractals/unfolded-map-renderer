...
export class OutputLoader {
  ...
  getData(): any {
    ...
    if (objectData !== undefined) {
      if (objectData.features) {
        // console.log('unfolded.map:data:format: GeoJSON');
    ...
    if (textData.length > 0) {
      console.log('unfolded.map:data:text:', textData.substring(0, Math.min(80, textData.length)), '...');
    ...
    // console.log('unfolded.map:data:output', this.outputData);
    const dataArray: Uint8Array = this.outputData.data();
    console.log(dataArray);
    if (dataArray.length > 0 ) {
      console.log(`unfolded.map:dataType: ${dataArray.constructor}`);
  ...
  getJsonData(data: any): any {
    // console.log('unfolded.map:data:json:', data);
    try {
      if (typeof data === 'string') {
        ...
        if (Array.isArray(objectData)) {
          console.log('unfolded.map:data:format: JSON array');
          return objectData;
        }
        else {
          console.log('unfolded.map:data:format: JSON');
          return objectData;
        }
      }
      ...
      console.log('unfolded.map:data: JSON.parse error:\n', error.message);
    }
    return undefined;
  }
  ...
  patchJson(data: string): string {
    ...
    // console.log('unfolded.map:data:text:', textData.substring(0, Math.min(300, textData.length)), '...');
    return textData;
  }
  ...
  isCsv(text: string): boolean {
    ...
    if (lines.length > 0) {
      console.log('unfolded.map:data:lines:', lines);
      const columns: string[] = lines[0].split(',');
      const columnCount = columns.length;

      if (columnCount > 1) {
        console.log('unfolded.map:data:columns:', columns);
        ...
        console.log('unfolded.map:data:format: CSV');
        return true;
      }
    }
    return false;
  ...
  xmlParse(xml: string): any {
    ...
    try {
      ...
      console.log('unfolded.map:data:format: XML');
      // console.log(JSON.stringify(jsonData, null, 2));
    }
    catch(error: any) {
      console.log('unfolded.map:data: XML parse error:\n', error.message);
    }
    return jsonData;
  }
  ...
  getGeoData(data: any): any {
    ...
    try {
      ...
      console.log('unfolded.map:data: GeoJSON parse error:\n', error);
    }
    if (!geoData.features || geoData.features.length <= 0) {
      // console.log(`unfolded.map:data:features: ${JSON.stringify(geoData.features, null, 2)}`);
